...
client.lavalink = new LavalinkManager({
    ...
    playerOptions: {
        ...
        onDisconnect: {
            ...
            reconnectOnReidentify: false, // if true: a voice-drop caused by a shard re-identify (fresh gateway IDENTIFY after a failed RESUME) re-handshakes voice so lavalink resumes the track, instead of destroying. real kicks/leaves still behave as configured above
            reidentifyWindowMs: 60_000, // how long (ms) after a shard READY a bot voice-drop is still treated as a reconnect artifact (only used with reconnectOnReidentify)
        },
        onEmptyQueue: {
